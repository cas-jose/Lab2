...
#include <EEPROM.h> //For read/write ESP8266
...
void setup() {
    ...
    sprintf(ap_ssid, "I Want To KMS", ESP.getChipId());
        ...
        case WStype_TEXT:
            ...
            if (payload[0] == '#') {
                ...
                { 
                  ...
                  forward();
                  char valu[20] = "Angle: ";
                  sprintf(valu, "Angle: %3f", headingDegrees);
                  wsSend(id, valu);
...
void SensorLoop()
{
  ...
  File h = SPIFFS.open("/frontSensor.txt", "a+");

  if (measurements2 < 100)
  {
    h.print("Front Distance: ");
    h.println(sensor.readRangeSingleMillimeters());
    measurements2  = measurements2 + 1;    
  }
   if (measurements2 == 100 && conti2 == true)
   {
    h.close();
    Serial.print("HERE");
    conti2 = false;
...
File k = SPIFFS.open("/rightSensor.txt", "a+");


  if (measurements3 < 100)
  {
    k.print("Front Sensor: ");
    k.println(sensor2.readRangeSingleMillimeters());
    measurements3  = measurements3 + 1;    
  }
   if (measurements3 == 100 && conti3 == true)
   {
    k.close();
    Serial.print("HERE");
    conti3 = false;
...
void MagnetometerLoop()
{
 ...
 heading = atan2(mx, my);
  ...
  float declinationAngle = 0.2083; // UCLA declination angle
...
File f = SPIFFS.open("/logData.txt", "a+");

  if (measurements1 < 100)
  {
    f.print("Angle: ");
    f.println(headingDegrees);
    measurements1  = measurements1 + 1;    
  }
   if (measurements1 == 100 && conti1 == true)
   {
    f.close();
    Serial.print("HERE");
    conti1 = false;
